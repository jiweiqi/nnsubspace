...
class NNSubspace:
    def __init__(self, model=None, x=None, x_train_mean=None, dataset_name=None):
        self.model = model
        self.x = x
    ...
    def sampling_setup(self, num_gradient_mc=10, num_rs_mc=10, seed=None, bool_clip=False, sigma=0, num_eigenvalue=10):
        ...
        if self.num_gradient_mc > self.num_rs_mc:
            raise Exception('num_rs_mc = should ne larger than num_gradient_mc = {}'.format(self.num_rs_mc,
                                                                                            self.num_gradient_mc))
    ...
    def gaussian(self):
        ...
        self.x_samples_rs = np.random.normal(loc=self.x,
                                             scale=self.sigma * np.ones_like(self.x),
                                             size=(
                                                 self.num_rs_mc, self.x.shape[1], self.x.shape[2],
                                                 self.x.shape[3]))
    ...
    def clip_gaussian(self):
        np.random.seed(self.seed)
        self.x_samples_rs = np.zeros((self.num_rs_mc,
        ...
        for i_sample in tqdm(range(x_flat.shape[0])):
            self.x_samples_rs[:, i_sample] = truncnorm.rvs(
                ...
                (1 - x_train_mean_flat[i_sample] - x_flat[i_sample]) / self.sigma,
                loc=x_flat[i_sample],
                scale=self.sigma,
                random_state=self.seed,
                size=self.num_rs_mc)

        self.x_samples_rs = self.x_samples_rs.reshape(self.num_rs_mc, self.x.shape[1], self.x.shape[2], self.x.shape[3])
        self.x_samples_gradient = self.x_samples_rs[0:self.num_gradient_mc]
    ...
    def sampling(self):
        ...
        self.x_samples_gradient_flat = self.x_samples_gradient.reshape(self.num_gradient_mc, -1)
    ...
    def svd(self):
        ...
        self.poly1d = np.poly1d(np.polyfit(self.xv[:, 0], self.y_samples_rs, 2))

        subspaceplot.summaryplot(self.xv[:, 0], self.y_samples_rs, self.poly1d, figsize=None)

    def evaluate_gradient(self):
        print('@evaluate_gradient')
        _start_time = datetime.now()

        self.gradients = K.gradients(self.model.output[0][self.i_class], self.model.input)
        ...
        for count, sample in tqdm(enumerate(self.x_samples_gradient)):
            if self.dataset_name == 'imagenet':
                evaluated_gradients = self.tf_session.run(self.gradients,
                                                          feed_dict={
                                                              self.model.input: np.expand_dims(sample * 255, axis=0)})
            else:
                evaluated_gradients = self.tf_session.run(self.gradients,
                                                          feed_dict={
                                                              self.model.input: np.expand_dims(sample, axis=0)})
            self.samples_gradient[count] = evaluated_gradients[0][0]

        _time_elapsed = datetime.now() - _start_time
        print('Time elapsed (hh:mm:ss.ms) {}'.format(_time_elapsed))

        self.samples_gradient_flat = self.samples_gradient.reshape(self.num_gradient_mc, -1)
